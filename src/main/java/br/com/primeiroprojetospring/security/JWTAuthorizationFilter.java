...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
		...
		String token = request.getHeader(ESecurityConstants.HEADER.desc());
		if (token == null || !token.startsWith(ESecurityConstants.TOKEN_PREFIX.desc())) {
	...
	private UsernamePasswordAuthenticationToken getAuthenticationToken(HttpServletRequest request, String token) {
		...
		String userName = Jwts.parser().setSigningKey(ESecurityConstants.SECRET.desc()).parseClaimsJws(token.replace(
				// Removendo Prefix "Bearer " que vem do HEADER,
				// deixando apenas o token
				ESecurityConstants.TOKEN_PREFIX.desc(), "")).getBody().getSubject();
