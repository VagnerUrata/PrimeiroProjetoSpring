...
//				.antMatchers(HttpMethod.GET, PUBLIC_MATCHERS_GET).permitAll().antMatchers(ADMIN_MATCHERS).permitAll()
//				.antMatchers(ADMIN_MATCHERS).hasRole(ADMIN).antMatchers(USER_MATCHERS).permitAll()
//				.antMatchers(USER_MATCHERS).hasRole(USER).antMatchers(USER_MATCHERS).hasAnyAuthority(PermissaoService.INSERT).anyRequest().authenticated().and().formLogin().permitAll()
