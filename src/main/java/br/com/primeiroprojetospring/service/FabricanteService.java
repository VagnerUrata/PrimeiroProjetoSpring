...
public class FabricanteService {
	...
	public Fabricante buscarFabricanteID(Integer id) {
		...
		return fabricante.orElseThrow(
				() -> new ObjectNotFoundException(new Fabricante(), "Fabricante não encontrado. Id: " + id));
	}
	...
	public List<Fabricante> findFabricanteForPais(String pais) {
		...
		return new JPAQueryFactory(entityManager).selectFrom(fabricante).where(fabricante.pais.eq(pais)).fetch();
	}
}