...
public class AcessorioService {
	...
	public Acessorio buscarAcessorioID(Integer id) {
		...
		return acessorio
				.orElseThrow(() -> new ObjectNotFoundException(new Acessorio(), "Acessorio não encontrado. Id: " + id));
	}
	...
	public List<Acessorio> findAcessorioStartWithAndEndWith(String inicial, String fim) {
		QAcessorio acessorio = QAcessorio.acessorio;

		return new JPAQueryFactory(entityManager).selectFrom(acessorio)
				.where(acessorio.nome.startsWith(inicial).and(acessorio.nome.endsWith(fim))).fetch();
	}

	public List<Acessorio> buscaPorNome(String nome) {
		return acessorioRepository.findByNomeAcessorioJPQL(nome);
	}
}